...
from db.database import init_db
from core.servces import DocumentService
...
if st.session_state.show_reset:
    ...
    if st.button("Confirm Reset"):
        if password_input == ADMIN_PASSWORD:
            ...
            # Delete DB
            db_file_dir = os.path.join("data", "documents")
            ...
            st.success("System reser Successfully. Reset app.")
            st.session_state.show_reset = False
            st.rerun()
        else:
...
with tabs[1]:
    ...
    if st.session_state.reader_mode and st.session_state.selected_doc:
        ...
        if not os.path.exists(image_dir):
            st.error("Images are not found. PDF conversion failed.")
        else:
           ...
           with col1:
               if st.button("<== Previous") and current_page > 0 :
                   analytics.record_app_visit("pevious_page")
...
with tabs[2]:
    ...
    if df.empty:
        st.info("No analytics data yet. Perform some action to see the insights")
    else:
